...
function effects(){
    ...
    document.getElementById("imageTag").style.filter=`blur(${blurValue}px) brightness(${brightnessVal}%) contrast(${constVal}%) grayscale(${grayVal}%) hue-rotate(${hueVal}deg) invert(${invVal}%) opacity(${opaVal}%) saturate(${satVal}%) sepia(${sepVal}%)`
}
